...
export class AddgroupComponent implements OnInit {
  ...
  onFunctionsSelected(checked: boolean, func: ModuleFunction) {
    ...
    const funcIndex = this.selectedFunctionsList.findIndex((selectedFunc: any) => selectedFunc.id === func.id);
    if (checked && funcIndex === -1) {
      // Function is selected and doesn't exist in the list, so add it
      this.selectedFunctionsList.push(func);
    } else if (!checked && funcIndex !== -1) {
      // Function is unselected and exists in the list, so remove it
      this.selectedFunctionsList.splice(funcIndex, 1);
    }
  }
  ...
  selectAllFunctions(event: any, submodule: SubModule) {
    submodule.functions.forEach((func: ModuleFunction) => {
      func.selected = event.target.checked;
      const funcIndex = this.selectedFunctions.findIndex((selectedFunc: ModuleFunction) => selectedFunc.id === func.id);
      if (event.target.checked && funcIndex === -1) {
        // Function is selected and doesn't exist in the list, so add it
        this.selectedFunctions.push(func);
      } else if (!event.target.checked && funcIndex !== -1) {
        // Function is unselected and exists in the list, so remove it
        this.selectedFunctions.splice(funcIndex, 1);
      }
    });
  }
